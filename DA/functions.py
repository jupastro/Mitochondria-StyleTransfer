...
def train(X_train,Y_train,X_val,Y_val,numEpochs,output_channels,patience,lr,min_lr,batch_size_value,schedule,model_name,optimizer_name,loss_acronym,max_pooling,train_encoder=True,random_encoder_weights=True,preTrain=False,Denoising=False,pre_load_weights=False,pretrained_model=None,plot_history=False,seg_weights=[1.,1.,5.],bottleneck_freezing=False,save_best_only=True):
  ...
  if model_name == 'UNet':
    ...
    model=Attention_U_Net_2D(image_shape = (None,None,1), activation='elu', feature_maps=[16, 32, 64, 128, 256],
                       ...
                       k_init='he_normal',num_outputs=1,pre_load_weights=False,pretrained_model=pretrained_model,train_encoder=train_encoder,bottleneck_train=bottleneck_train,skip_connection_train=True)
...
def Attention_U_Net_2D(image_shape = (None,None,1), activation='elu', feature_maps=[16, 32, 64, 128, 256],
                       drop_values=[0.1,0.1,0.2,0.2,0.3], spatial_dropout=False, batch_norm=False,
                       k_init='he_normal',num_outputs=1,pre_load_weights=False,pretrained_model=None,train_encoder=True,bottleneck_train=True,skip_connection_train=True):
